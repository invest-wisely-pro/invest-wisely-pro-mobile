...
function loadSimulator() {
  ...
  ['fmt','fmtN','fmtFull','getCrashYears',
   ...
   'getGlideAlpha','strategyToSlots','getGlideSlots','getGlideParams','_glideSig','_glideSideObj','addGlideSlot','delGlideSlot','updGlideSlotAc','updGlideSlotPct','setGlideSide'].forEach(fn => loadFn(SRC.main, fn));
  if (SRC.bt) { try { loadFn(SRC.bt, 'glideIsNonBacktestable'); } catch(e){} }
  if (SRC.quant) {
    for (const re of [/const RF_RATE\s*=\s*[^;]+;/, /const PRESET_LEVERAGE\s*=\s*\{[\s\S]*?\};/, /const PRESET_COMPOSITION\s*=\s*\{[\s\S]*?\n\};/]) {
      const m = SRC.quant.match(re); if (m) { try { eval(m[0].replace('const ', 'global.')); } catch(e){} }
    }
    for (const fn of ['_normCDF','_normInv','_erf','_tPPF','_tCVaR','portfolioMu','portfolioVar','getCurrentPortfolioPoint','computeVaRCVaR','computeEfficientFrontier','findMaxSharpe','findMinVariance']) { try { loadFn(SRC.quant, fn); } catch(e){} }
  }
  { const gc = grab(SRC.main, /let _glideCache = \{[\s\S]*?\};/); if (gc) eval(gc.replace('let ', 'global.')); }
}
function setState(o) {
...
function suiteFactors() {
  ...
  // 7.s GLIDE PATH: semantica curvatura + de-risking monotono
  try {
    setState({ portfolio: 'glide', age: 30 });
    // alpha: estremi e monotonia
    ok(Math.abs(global.getGlideAlpha(30,30,65,2)-1) < 1e-9 && Math.abs(global.getGlideAlpha(65,30,65,2)) < 1e-9, 'Glide: alpha=1 a start, 0 a end');
    // k>1 = de-risk TARDIVO (resta aggressivo): a meta orizzonte alpha>0.7
    ok(global.getGlideAlpha(47.5,30,65,2) > 0.7, 'Glide: k=2 de-risk tardivo (coerente col rischio di sequenza)', global.getGlideAlpha(47.5,30,65,2).toFixed(3));
    ok(Math.abs(global.getGlideAlpha(47.5,30,65,1)-0.5) < 0.02, 'Glide: k=1 lineare');
    ok(global.getGlideAlpha(47.5,30,65,0.5) < 0.4, 'Glide: k<1 de-risk precoce');
    // equity decresce con eta (de-risking)
    const e30 = global.getGlideParams(30).eq, e50 = global.getGlideParams(50).eq, e65 = global.getGlideParams(65).eq;
    ok(e30 > e50 && e50 > e65 && e30 > 0.7 && Math.abs(e65-0.40) < 0.12, 'Glide: equity decresce 30>50>65 (ec_glob->golden_butterfly)', (e30*100).toFixed(0)+'/'+(e50*100).toFixed(0)+'/'+(e65*100).toFixed(0)+'%');
    // slots sommano ~100 e usano AC validi
    const sl = global.getGlideSlots(50);
    const tot = sl.reduce((s,x)=>s+x.pct,0);
    ok(Math.abs(tot-100) < 2 && sl.every(x=>global.ASSET_CLASSES[x.ac]), 'Glide: slots@50 somma ~100, AC validi', tot.toFixed(1));
    // lati CUSTOM: il glide accetta strategie custom (non solo preset)
    global.state.glide = { sideA:{type:'custom',slots:[{ac:'eq_usa',pct:100}]}, sideB:{type:'custom',slots:[{ac:'ob_glob_agg',pct:70},{ac:'gold',pct:30}]}, ageStart:30, ageEnd:65, k:2 };
    if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
    const cA = global.getGlideParams(30), cB = global.getGlideParams(65);
    ok(isFinite(cA.eq) && cA.eq > 0.9 && isFinite(cB.eq) && cB.eq < 0.1, 'Glide: lati CUSTOM funzionano (100% USA -> 70 bond/30 oro)', (cA.eq*100).toFixed(0)+'%->'+(cB.eq*100).toFixed(0)+'%');
    // custom vuoto: fallback sicuro senza NaN
    global.state.glide.sideA = { type:'custom', slots:[] };
    if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
    ok(isFinite(global.getGlideParams(40).eq), 'Glide: lato custom vuoto -> fallback finito (no NaN)');
    // decumulo: il glide continua a de-rischiare verso B man mano che l'età avanza
    global.state.glide = { sideA:{type:'preset',ref:'eq80'}, sideB:{type:'preset',ref:'golden_butterfly'}, ageStart:30, ageEnd:75, k:2 };
    if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
    const dEq65 = global.getGlideParams(65).eq, dEq70 = global.getGlideParams(70).eq, dEq75 = global.getGlideParams(75).eq;
    ok(dEq65 > dEq70 && dEq70 > dEq75, 'Glide decumulo: de-risking continua 65>70>75 (transizione verso B in fase prelievo)', (dEq65*100).toFixed(0)+'>'+(dEq70*100).toFixed(0)+'>'+(dEq75*100).toFixed(0));
    // getRate glide nel decumulo usa l'età che avanza (decStartAge + year)
    const gr0 = global.getRate('glide','normal',0,65), gr10 = global.getRate('glide','normal',10,65);
    ok(isFinite(gr0) && isFinite(gr10), 'Glide decumulo: getRate finito lungo il prelievo');
    // GATE: glide con lato a leva/trend -> non-backtestable (blocco backtest storico + block bootstrap MC)
    if (typeof global.glideIsNonBacktestable === 'function') {
      global.state.portfolio = 'glide';
      global.state.glide = { sideA:{type:'preset',ref:'ec_glob_9060'}, sideB:{type:'preset',ref:'golden_butterfly'}, ageStart:30, ageEnd:65, k:2 };
      ok(global.glideIsNonBacktestable() === true, 'Glide GATE: lato Efficient Core (leva) -> non-backtestable');
      global.state.glide = { sideA:{type:'preset',ref:'eq80'}, sideB:{type:'preset',ref:'golden_butterfly'}, ageStart:30, ageEnd:65, k:2 };
      ok(global.glideIsNonBacktestable() === false, 'Glide GATE: 80/20 -> GB backtestabile (no blocco inutile)');
    } else warn('Glide GATE: glideIsNonBacktestable non caricata');
    // QUANT: frontiera/optimizer/VaR-CVaR col glide (semplice e a leva)
    if (typeof global.getCurrentPortfolioPoint === 'function' && typeof global.computeVaRCVaR === 'function') {
      global.state.portfolio = 'glide'; global.state.age = 38;
      global.state.glide = { sideA:{type:'preset',ref:'eq80'}, sideB:{type:'preset',ref:'golden_butterfly'}, ageStart:30, ageEnd:65, k:2 };
      if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
      const qp = global.getCurrentPortfolioPoint(0.2);
      ok(qp && isFinite(qp.mu) && isFinite(qp.vol) && qp.vol > 0 && isFinite(qp.sharpe), 'Glide QUANT: punto frontiera efficiente finito');
      // con lato a leva: il punto resta finito (usa parametri motore, no NaN dai compositi)
      global.state.glide = { sideA:{type:'preset',ref:'ec_glob_9060'}, sideB:{type:'preset',ref:'golden_butterfly'}, ageStart:30, ageEnd:65, k:2 };
      if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
      const qpL = global.getCurrentPortfolioPoint(0.2);
      ok(qpL && isFinite(qpL.mu) && isFinite(qpL.vol) && qpL.vol > 0, 'Glide QUANT: punto frontiera finito anche con lato Efficient Core (leva)');
      // NOTA: VaR/CVaR completo verificato in test_quant_glide.js (contesto completo)
    } else warn('Glide QUANT: funzioni quant non caricate');
    // due custom INDIPENDENTI: modificare A non tocca B
    global.state.glide = { sideA:{type:'custom',slots:[{ac:'eq_usa',pct:100}]}, sideB:{type:'custom',slots:[{ac:'gold',pct:100}]}, ageStart:30, ageEnd:65, k:2 };
    if (typeof global._glideCache !== 'undefined') global._glideCache.sig = null;
    if (typeof global.addGlideSlot === 'function') {
      global.render = global.render || function(){};
      const bBefore = global.state.glide.sideB.slots.length;
      global.addGlideSlot('a');
      ok(global.state.glide.sideB.slots.length === bBefore, 'Glide: due custom indipendenti (modifica A non tocca B)');
      const gA = global.getGlideParams(30), gB = global.getGlideParams(65);
      ok(isFinite(gA.eq) && gA.eq > 0.9 && isFinite(gB.eq) && gB.eq < 0.1, 'Glide: due custom distinti calcolano (100% USA -> 100% oro)');
    }
  } catch(e){ warn('Glide: non verificabile — ' + e.message.slice(0,40)); }

  // 7.t GLIDE nel decumulo parametrico: getRate finito lungo tutto l'orizzonte di prelievo
  // (il MC avanzato e il decumulo parametrico usano getRate per il rendimento per eta;
  // il decumulo STORICO single-sequence e' invece bloccato con messaggio, come efficient core).
  try {
    setState({ portfolio: 'glide', age: 30, years: 35 });
    let allFin = true;
    for (let y = 0; y <= 30; y += 5) {
      for (const sc of ['worst','normal','best']) {
        if (!isFinite(global.getRate('glide', sc, y, 65))) allFin = false;
      }
    }
    ok(allFin, 'Glide getRate: finito per worst/normal/best lungo il decumulo (eta 65->95)');
    // ordinamento scenari coerente a eta fissa
    const w = global.getRate('glide','worst',5,65), n = global.getRate('glide','normal',5,65), b = global.getRate('glide','best',5,65);
    ok(w <= n && n <= b, 'Glide getRate: worst<=normal<=best', [w,n,b].map(x=>x.toFixed(3)).join('/'));
  } catch(e){ warn('Glide decumulo: non verificabile — ' + e.message.slice(0,40)); }
}
