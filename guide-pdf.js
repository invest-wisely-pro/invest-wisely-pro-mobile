...
async function downloadGuidePDF() {
  ...
  try {
    ...
    h3('Glide Path — transizione continua tra due strategie');
    p('Il Glide Path generalizza il Lifecycle: invece di interpolare solo la quota azionaria, interpola simultaneamente tutte le proprieta di due strategie complete. Imposti una strategia A di partenza (tipicamente aggressiva), una strategia B di arrivo (conservativa), l\'eta di inizio e di fine, e una curvatura k. Lungo l\'orizzonte, quota azionaria, leva, composizione, fiscalita ed esposizione al cambio transitano insieme da A verso B.');
    p('La curvatura k governa la forma della transizione: k=1 e lineare; k>1 mantiene la strategia aggressiva piu a lungo per poi de-rischiare rapidamente vicino alla meta (forma coerente col rischio di sequenza, che colpisce di piu negli ultimi anni prima dell\'obiettivo, quando il capitale esposto e massimo e resta poco tempo per recuperare); k<1 de-rischia presto. Il backtest storico su singola sequenza non e disponibile per il Glide Path, perche la composizione cambia ogni anno e i lati possono includere leva: usa il Monte Carlo Avanzato, che simula correttamente la transizione per eta.');
